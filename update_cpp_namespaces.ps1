...
function Update-CppFile {
    ...
    if (-not (Test-Path $FilePath)) {
        if ($Verbose) { Write-Host "[WARN]  Not found: $FilePath" -ForegroundColor Yellow }
        return $false
    }
...
if ($DryRun) {
    Write-Host "[OK] Dry run complete. Run without -DryRun to apply changes." -ForegroundColor Green
} else {
    Write-Host "[OK] Implementation file update complete!" -ForegroundColor Green
}
