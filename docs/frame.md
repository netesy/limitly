...
* [OK] **Lifecycle management** (init/deinit)
* [OK] **Sequential by default** (opt-in concurrency)
...
**Characteristics**:
- [OK] Fork-join parallelism (CPU-bound)
- [OK] Direct code execution with `iter()`
- [OK] exclusive slice capabilities for mutable access
- [OK] Synchronous parallel execution
- [OK] No channels required
...
**Characteristics**:
- [OK] Bounded concurrency (fixed iteration range)
- [OK] Task-based syntax: `task(i in range)`
- [OK] Channel-based output
- [OK] Atomic operations on shared state
- [OK] Auto-closes channel when complete
- [OK] Good for parallelizing bounded work over I/O
...
**Characteristics**:
- [OK] Unbounded concurrency (event stream)
- [OK] Worker-based syntax: `worker(event)`
- [OK] Channel-based input/output
- [OK] Event-driven processing
- [OK] Runs until stream exhausted or timeout
- [OK] Good for continuous I/O streams
...
**Composition Rules**:
- [OK] Frames can embed other frames as fields
- [OK] Embedded frames auto-destroyed when parent destroyed
- [OK] Destruction order: children first, then parent
- [OK] Cross-mode composition allowed (sequential ↔ parallel ↔ concurrent)
...
frame SecureProcessor : Processor {
    ...
    parallel(cores=4) {
        iter(i in 0..99) {
            // self.encryption_key  // [FAIL] compile error: private
            self.temp_buffer[i] = compute(i)  // [OK] OK: protected
            self.results[i] = self.temp_buffer[i] * 2  // [OK] OK: public
        }
    }
    ...
    concurrent(ch=out_ch, mode=batch, cores=4) {
        task(i in 0..99) {
            // self.encryption_key  // [FAIL] compile error: private
            self.temp_buffer[i] = process(i)  // [OK] OK: protected
            out_ch.send(self.results[i])  // [OK] OK: public
        }
    }
}
```
...
frame Service : Configurable {
    ...
    pub fn get_config(): Config {
        return self.config  // [OK] trait method accessing protected field
    }
...
|-------|--------|--------------|----------|----------|-------|
| **Parallel** | `iter(i in range)` | linear slices | CPU-bound computation | [OK] Yes | Range/array |
| **Concurrent (batch)** | `task(i in range)` | Channels + atomic | I/O-bound batch work | [OK] Yes | Range/array |
| **Concurrent (stream)** | `worker(event)` | Channels + atomic | Continuous I/O streams | [FAIL] No | Event stream |
...
[OK] Full encapsulation (default private, `prot`, `pub`)  
[OK] Trait-based polymorphism (static + dynamic)  
[OK] Automatic resource management  
[OK] Compile-time concurrency safety  
[OK] Three clear concurrency models  
[OK] Cross-mode composition  
[OK] Error resilience  
[OK] Channel auto-closing  
