...
* [x] **Composition over inheritance** (no frame hierarchies)
* [x] **Trait-based polymorphism** (interfaces)
* [x] **Encapsulation** with visibility modifiers
* [x] **Concurrency integration** (parallel/concurrent)
* [x] **Lifecycle management** (init/deinit)
* [x] **Sequential by default** (opt-in concurrency)
...
```limit
frame <FrameName> {
    // Fields (default: private if no modifier, must end with semicolon)
...
**Characteristics**:
- [x] Fork-join parallelism (CPU-bound)
- [x] Direct code execution with `iter()`
- [x] exclusive slice capabilities for mutable access
- [x] Synchronous parallel execution
- [x] No channels required
...
**Characteristics**:
- [x] Bounded concurrency (fixed iteration range)
- [x] Task-based syntax: `task(i in range)`
- [x] Channel-based output
- [x] Atomic operations on shared state
- [x] Auto-closes channel when complete
- [x] Good for parallelizing bounded work over I/O
...
**Characteristics**:
- [x] Unbounded concurrency (event stream)
- [x] Worker-based syntax: `worker(event)`
- [x] Channel-based input/output
- [x] Event-driven processing
- [x] Runs until stream exhausted or timeout
- [x] Good for continuous I/O streams
...
**Composition Rules**:
- [x] Frames can embed other frames as fields
- [x] Embedded frames auto-destroyed when parent destroyed
- [x] Destruction order: children first, then parent
- [x] Cross-mode composition allowed (sequential <-> parallel <-> concurrent)
...
frame SecureProcessor : Processor {
 ...
 self.temp_buffer[i] = compute(i) // [x] OK: protected
 self.results[i] = self.temp_buffer[i] * 2 // [x] OK: public
 }
 }
 ...
 task(i in 0..99) {
 // self.encryption_key // [ ] compile error: private
 self.temp_buffer[i] = process(i) // [x] OK: protected
 out_ch.send(self.results[i]) // [x] OK: public
 }
 }
}
```
...
frame Service : Configurable {
 ...
 return self.config // [x] trait method accessing protected field
 }
...
|-------|--------|--------------|----------|----------|-------|
| **Parallel** | `iter(i in range)` | linear slices | CPU-bound computation | [x] Yes | Range/array |
| **Concurrent (batch)** | `task(i in range)` | Channels + atomic | I/O-bound batch work | [x] Yes | Range/array |
...
[x] Full encapsulation (default private, `prot`, `pub`) 
[x] Trait-based polymorphism (static + dynamic) 
[x] Automatic resource management 
[x] Compile-time concurrency safety 
[x] Three clear concurrency models 
[x] Cross-mode composition 
[x] Error resilience 
[x] Channel auto-closing 
