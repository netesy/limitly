...
> [WARN] Note: The `data` keyword modifier has been removed from frame declarations in favor of traits and standard frames.
...
> [WARN] Note: `unsafe` blocks are currently disabled by the compiler pending complete memory-model boundary validation.
...
> [WARN] Note: `contract(...)` statements are parsed as reserved keywords but lowerings are currently unimplemented in LIR.
...
> [WARN] Note: `comptime` blocks are currently disabled by the compiler.
