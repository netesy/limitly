...
> **Note:** A standard library with collections like Lists and Dictionaries is planned but not yet implemented.
...
> **Note:** Basic class syntax is implemented, but inheritance and other advanced features are not yet complete.
...
> **Note:** The module system syntax is parsed by the compiler, but the VM and runtime do not yet support module loading or linking.
...
> **Note:** Advanced type system features like intersection types, refined types, and structural types are planned but not yet implemented.
...
For convenience, Limit provides the `Type?` syntax as a shorthand for fallible operations.
...
```limit
// This is a conceptual example. A real `to_int` function is not yet in the standard library.
...
> **Note:** The concurrency syntax (`parallel`, `concurrent`, `task`) is parsed by the compiler, but the VM and runtime do not yet support concurrent execution.
