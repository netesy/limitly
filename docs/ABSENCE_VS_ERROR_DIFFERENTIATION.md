...
- [OK] Result is simply "nothing" or "not applicable"
- [OK] Represented as `nil` or union with `nil`
- [OK] Predictable and expected outcome
...
- [FAIL] Cannot produce expected result
- [FAIL] Represented as `err()` or `err(ErrorType)`
- [FAIL] Unexpected and exceptional outcome
...
[OK] There's simply no value to return
[OK] The absence is an expected outcome
[OK] Examples:
- Searching for an item that doesn't exist
...
fn divide(a: int, b: int): int?DivisionByZero {
    if (b == 0) {
        return err(DivisionByZero);  // [OK] Correct
    }
    return ok(a / b);
}
...
fn process(): int?DivisionByZero {
    var result = divide(10, 2)?;  // [OK] Compatible error type
    return ok(result);
}

// Incompatible error types are caught
fn incompatible(): int?IndexOutOfBounds {
    var result = divide(10, 2)?;  // [FAIL] Error: DivisionByZero not compatible
    return ok(result);
}
```
