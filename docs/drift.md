...
## [WARN] Documentation Drift (Consistency Errors)
...
| :--- | :--- | :--- |
| "Explicit is better than implicit" | Enforced in `TypeChecker::is_type_compatible` (decimals). | [OK] |
| "Errors are not exceptions" | Implemented via `Type?` system. | [OK] |
| "Structured Concurrency" | Implemented via `parallel`/`concurrent` blocks. | [OK] |
| "Absence of value handled explicitly" | defines `nil` and `Type?`, but `nil` exists. | [OK] |
