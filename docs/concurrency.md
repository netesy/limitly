...
The currently available deterministic reduction pattern is: publish one value
per source iteration to a channel, join the structured block, then fold that
channel in source iteration order. This is not yet a canonical parallel
reduction operation: current backends do not implement a shared reduction tree
or parallel ordered-commit buffer. Atomic scalars remain synchronization
primitives, not deterministic reductions.
