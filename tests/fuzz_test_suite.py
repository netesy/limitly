...
def main():
    ...
    if not os.path.exists(compiler_path):
        print_color(f"[FAIL] Error: Limitly executable not found at {compiler_path}", COLOR_RED)
        print("Please build the compiler first by running 'make' or './build.sh'.")
        sys.exit(1)

    print_color("======================================================================", COLOR_CYAN)
    print_color("      [RUN] STARTING LIMITLY FUZZY TEST SUITE (WITH HUMAN READABILITY)    ", COLOR_BOLD + COLOR_CYAN)
    print_color("======================================================================", COLOR_CYAN)
    ...
    if not seeds:
        print_color("[FAIL] Error: No seed .lm files found in seeds directory!", COLOR_RED)
        sys.exit(1)
    ...
    if runner.failed_count > 0:
        print_color("\n[FAIL] Some fuzzy tests exposed issues or hangs in the compiler. See above diagnostics.", COLOR_RED)
        sys.exit(1)
    else:
