...
run_test() {
  ...
  if grep -qiE "segmentation fault|segfault" "$tmp"; then
    ...
    ((FAILED+=1))
  elif grep -qE "error\[E|Error:|RuntimeError|SemanticError|BytecodeError|[FAIL] FAIL|ASSERT.*FAIL|Assertion.*failed" "$tmp"; then
    echo "  FAIL: $f"
    grep -E "error\[E|Error:|RuntimeError|SemanticError|BytecodeError|[FAIL] FAIL|ASSERT.*FAIL|Assertion.*failed" "$tmp" | head -n 3
    ((FAILED+=1))
  else
    echo "  PASS: $f"
    ((PASSED+=1))
