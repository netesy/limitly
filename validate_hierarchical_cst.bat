...
if not exist %PARSER% (
    echo [FAIL] Parser executable not found: %PARSER%
...
if %ERRORLEVEL% neq 0 (
    echo [FAIL] If blocks parsing failed
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No IF_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No BLOCK_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No PRINT_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo [FAIL] Loops parsing failed
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No FOR_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No WHILE_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No ITER_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo [FAIL] Functions parsing failed
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No FUNCTION_DECLARATION nodes found
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo [FAIL] Classes parsing failed
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No CLASS_DECLARATION nodes found
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    echo [FAIL] Expressions parsing failed
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No EXPRESSION_STATEMENT nodes found
    exit /b 1
)
...
if %ERRORLEVEL% neq 0 (
    ...
    echo [FAIL] Found print statements at root level - hierarchy broken
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    ...
    echo [FAIL] No nesting indicators found - flat structure detected
    exit /b 1
)
...
if exist %TEST_DIR%\hierarchical_if_blocks.lm.cst.txt (
    ...
    echo [FAIL] CST output files not generated
    exit /b 1
)
...
for %%f in (%TEST_DIR%\*.lm.cst.txt) do (
    for %%s in ("%%f") do (
        if %%~zs gtr 1000 (
            ...
            echo [FAIL] %%~nxf is too small (%%~zs bytes) - may be incomplete
            exit /b 1
        )
    )
)
