...
function Add-NamespaceToHeader {
    ...
    if ($content -match "namespace LM") {
        Write-Host "[WARN]  Already has namespace: $FilePath" -ForegroundColor Yellow
        return $false
    }
...
Write-Host ""
if ($DryRun) {
    Write-Host "[OK] Dry run complete. Use without -DryRun to apply changes." -ForegroundColor Green
} else {
    Write-Host "[OK] Namespace addition complete!" -ForegroundColor Green
}
