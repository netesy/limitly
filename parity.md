...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `hello_lm` | Limitly (Fyra AOT) | Default (`-O2`) | **1,083,880 B (~1.08 MB)** | 18,702 B | **1,048,778 B** | 0 B | ~1,067,480 B |
...
The Limitly Fyra AOT executable is **~75x larger** on disk than a GCC-compiled C executable. Detailed ELF section analysis (`readelf -S` and `objdump`) reveals four primary causes for this size inflation:

### Cause 1: 1MB Static Heap Buffer Emitted into `.data` (`SHT_PROGBITS`) Instead of `.bss` (`SHT_NOBITS`)
...
- **Mechanism**:
  ...
  However, in `CodeGen.cpp` (in-memory assembler code path), the 1MB buffer of zeroes is emitted into `rodataAssembler` using a 1,048,576-byte loop (`rodataAssembler->emitByte(0)`).
  This places the 1MB zeroes array into the `.data` section as `SHT_PROGBITS`.
  ...
  In contrast, in standard C/C++ compilers, zero-initialized buffers are placed in the `.bss` section marked `SHT_NOBITS`, which takes **0 bytes on disk** and is allocated at runtime by the OS kernel loader.
...
- **Impact**: **+15 KB - 20 KB**
- **Mechanism**:
  Fyra currently embeds all standard built-in functions into the generated ELF binary's `.text` section regardless of whether they are referenced by `main()`.

---

## 3. Detailed Steps & Plan to Achieve Parity with GCC

To reduce Limitly Fyra AOT binary size from **1.08 MB** down to **~14 KB** (achieving 1:1 parity with GCC), execute the following technical plan across three phases:

### Phase 1: Eliminate the 1MB Data Payload (Immediate Reduction to ~30 KB)
...
2. **Update ELF Generator `vendor/fyra/src/target/artifact/executable/elf.cpp`**:
   ...
   - **Expected Result**: Binary file size drops from **1,083,880 B** to **~35,000 B**.

### Phase 2: Optimize File Layout & Alignment (Reduction to ~18 KB)
...
2. **Combine Read-Only Data (`.rodata`) into Code (`.text`) Segment**:
   ...
   - **Expected Result**: Binary file size drops to **~20,000 B**.

### Phase 3: Binary Stripping & Dead Code Elimination (Reduction to ~14 KB - GCC Parity)
...
2. **Implement Link-Time / CodeGen Dead Code Elimination (DCE)**:
   ...
   - **Expected Result**: Final stripped executable size **~14 KB - 15 KB** (100% parity with `gcc -O2 -s`).
...
- [ ] Phase 1: Convert `__fyra_heap` to `.bss` / `SHT_NOBITS` (Target: ~35 KB)
