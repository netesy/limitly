...
void MemoryChecker::check_var_declaration(std::shared_ptr<LM::Frontend::AST::VarDeclaration> var_decl) {
    if (!var_decl) return;
    if (var_decl->inferred_type) variable_types[var_decl->name] = var_decl->inferred_type;
...
void MemoryChecker::infer_parallel_capabilities(
    ...
    for (const auto& statement : parallel_stmt->body->statements) {
        ...
        for (const auto& collection : written_collections) {
            ...
            formal_type.indexVariable = index_name;
            formal_type.begin = capability.begin;
            formal_type.end = capability.end;
            formal_type.mutableAccess = true;
            if (auto type = variable_types.find(collection); type != variable_types.end()) {
                formal_type.collectionType = type->second;
                formal_type.valueType = type->second;
                if (const auto* list = std::get_if<ListType>(&type->second->extra)) {
                    formal_type.valueType = list->elementType;
                }
            }
