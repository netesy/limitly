...
void FyraBuiltinFunctions::emit_used_builtins(ir::Module* module,
    ...
    if (used_builtins.count("lm_str_eq")) emit_str_eq_ir(module, builder);
...
ir::Value* FyraBuiltinFunctions::emit_decimal_to_str_inline(ir::Module* module,
    ...
    ir::Instruction* buf_ptr = builder->createExternCall("memory.alloc", {ctx->getConstantInt(i64, 64)}, ptr_i8);
    ...
    ir::BasicBlock* b_neg  = builder->createBasicBlock("dts_neg_" + s_uid, cur_fn);
    ir::BasicBlock* b_abs  = builder->createBasicBlock("dts_abs_" + s_uid, cur_fn);
    ir::BasicBlock* b_prep = builder->createBasicBlock("dts_prep_" + s_uid, cur_fn);
    ir::BasicBlock* b_frac_loop = builder->createBasicBlock("dts_frac_loop_" + s_uid, cur_fn);
    ir::BasicBlock* b_dot  = builder->createBasicBlock("dts_dot_" + s_uid, cur_fn);
    ir::BasicBlock* b_whole_loop = builder->createBasicBlock("dts_whole_loop_" + s_uid, cur_fn);
    ir::BasicBlock* b_minus = builder->createBasicBlock("dts_minus_" + s_uid, cur_fn);
    ir::BasicBlock* b_done = builder->createBasicBlock("dts_done_" + s_uid, cur_fn);
    ...
    builder->createBr(w_more, b_whole_loop, b_minus);

    builder->setInsertPoint(b_minus);
    ir::Value* was_neg = builder->createLoad(v_is_neg);
    ir::BasicBlock* b_write_minus = builder->createBasicBlock("dts_wm_" + s_uid, cur_fn);
    builder->createBr(builder->createCeq(was_neg, ctx->getConstantInt(i64, 1)), b_write_minus, b_done);

    builder->setInsertPoint(b_write_minus);
...
ir::Value* FyraBuiltinFunctions::emit_int_to_str_inline(ir::Module* module,
    ...
    ir::BasicBlock* b_neg  = builder->createBasicBlock("its_neg_" + s_uid, cur_fn);
    ir::BasicBlock* b_abs  = builder->createBasicBlock("its_abs_" + s_uid, cur_fn);
    ir::BasicBlock* b_loop = builder->createBasicBlock("its_loop_" + s_uid, cur_fn);
    ir::BasicBlock* b_minus = builder->createBasicBlock("its_minus_" + s_uid, cur_fn);
    ir::BasicBlock* b_done = builder->createBasicBlock("its_done_" + s_uid, cur_fn);
    ...
    builder->createStore(ctx->getConstantInt(i64, 1), v_is_neg);
    ir::Value* neg_v = builder->createNeg(val);
    builder->createStore(neg_v, v_val);
    builder->createJmp(b_loop);
    ...
    builder->setInsertPoint(b_loop);
    ir::Value* cur_v = builder->createLoad(v_val);
    ir::Value* d = builder->createRem(cur_v, ctx->getConstantInt(i64, 10));
    ir::Value* next_v = builder->createDiv(cur_v, ctx->getConstantInt(i64, 10));
    builder->createStore(next_v, v_val);
    ...
    ir::Value* more = builder->createCsge(next_v, ctx->getConstantInt(i64, 1));
    builder->createBr(more, b_loop, b_minus);

    builder->setInsertPoint(b_minus);
    ir::Value* was_neg = builder->createLoad(v_is_neg);
    ir::BasicBlock* b_write_minus = builder->createBasicBlock("its_wm_" + s_uid, cur_fn);
    builder->createBr(builder->createCeq(was_neg, ctx->getConstantInt(i64, 1)), b_write_minus, b_done);
...
ir::Value* FyraBuiltinFunctions::emit_float_to_str_inline(ir::Module* module,
    ...
    ir::Instruction* buf = builder->createExternCall("memory.alloc", {ctx->getConstantInt(i64, 64)}, ptr_i8);
    ir::GlobalVariable* fmt_gv = get_or_create_global_str(module, builder, "fmt_float", "%g");
...
ir::Value* FyraBuiltinFunctions::emit_bool_to_str_inline(ir::Module* module,
    ...
    std::string uid = std::to_string(++g_print_str_counter);
    ir::BasicBlock* b_true = builder->createBasicBlock("bts_true_" + uid, cur_fn);
    ir::BasicBlock* b_false = builder->createBasicBlock("bts_false_" + uid, cur_fn);
    ir::BasicBlock* b_done = builder->createBasicBlock("bts_done_" + uid, cur_fn);

    ir::Instruction* res_slot = builder->createAlloc(ctx->getConstantInt(i64, 8), i64);

    ir::Value* is_t = builder->createCeq(val, ctx->getConstantInt(i64, 1));
    builder->createBr(is_t, b_true, b_false);
...
void FyraBuiltinFunctions::emit_str_eq_ir(ir::Module* module, ir::IRBuilder* builder) {
    if (module->getFunction("lm_str_eq")) {
        ir::Function* existing = module->getFunction("lm_str_eq");
        if (!existing->getBasicBlocks().empty()) return;
    }

    auto ctx = module->getContextShared();
    auto i64 = ctx->getIntegerType(64);
    auto i32 = ctx->getIntegerType(32);
    auto p8  = ctx->getPointerType(ctx->getIntegerType(8));

    ir::Function* fn = module->getFunction("lm_str_eq");
    if (!fn) fn = builder->createFunction("lm_str_eq", i64, {p8, p8});

    ir::BasicBlock* b_entry = builder->createBasicBlock("str_eq_entry", fn);
    ir::BasicBlock* b_null  = builder->createBasicBlock("str_eq_null",  fn);
    ir::BasicBlock* b_loop  = builder->createBasicBlock("str_eq_loop",  fn);
    ir::BasicBlock* b_next  = builder->createBasicBlock("str_eq_next",  fn);
    ir::BasicBlock* b_true  = builder->createBasicBlock("str_eq_true",  fn);
    ir::BasicBlock* b_false = builder->createBasicBlock("str_eq_false", fn);
    ...
    ir::Value* ptr_eq = builder->createCeq(s1, s2);
    builder->createBr(ptr_eq, b_true, b_null);

    builder->setInsertPoint(b_null);
    ir::Value* s1_null = builder->createCeq(s1, ctx->getConstantInt(i64, 0));
    ir::Value* s2_null = builder->createCeq(s2, ctx->getConstantInt(i64, 0));
    ir::Value* any_null = builder->createOr(s1_null, s2_null);

    ir::Instruction* idx_alloc = builder->createAlloc(ctx->getConstantInt(i64, 8), i64);
    builder->createStore(ctx->getConstantInt(i64, 0), idx_alloc);
    builder->createBr(any_null, b_false, b_loop);

    builder->setInsertPoint(b_loop);
    ir::Value* idx = builder->createLoad(idx_alloc);
    ir::Value* p1 = builder->createAdd(s1, idx);
    ir::Value* c1 = builder->createLoadub(p1);
    ir::Value* p2 = builder->createAdd(s2, idx);
    ir::Value* c2 = builder->createLoadub(p2);

    ir::Value* char_neq = builder->createCne(c1, c2);
    builder->createBr(char_neq, b_false, b_next);

    builder->setInsertPoint(b_next);
    ir::Value* is_zero = builder->createCeq(c1, ctx->getConstantInt(i32, 0));
    ir::Value* next_idx = builder->createAdd(idx, ctx->getConstantInt(i64, 1));
    builder->createStore(next_idx, idx_alloc);
    builder->createBr(is_zero, b_true, b_loop);
