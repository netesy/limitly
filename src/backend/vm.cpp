...
ValuePtr VM::execute(const std::vector<Instruction>& code) {
    ...
    try {
        ...
        while (ip < bytecodeRef.size()) {
            ...
            if (debugMode) {
                std::cout << "[DEBUG] Executing instruction " << ip << ": opcode " << static_cast<int>(instruction.opcode) << std::endl;
            }
            ...
            if (insideFunctionDefinition && instruction.opcode != Opcode::END_FUNCTION) {
                if (debugMode) {
                    std::cout << "[DEBUG] Skipping instruction " << ip << " (inside function definition)" << std::endl;
                }
                ip++;
                continue;
            }
...
void VM::handleCall(const Instruction& instruction) {
    ...
    if (debugMode) {
        std::cout << "[DEBUG] Function call: " << funcName << " with " << argCount << " args" << std::endl;
    }
   ...
   // ValuePtr callee = nullptr;
    ...
    auto funcIt = userDefinedFunctions.find(funcName);
    if (debugMode) {
        std::cout << "[DEBUG] Looking for user-defined function: " << funcName << std::endl;
...
void VM::handleBeginFunction(const Instruction& instruction) {
    ...
    if (debugMode) {
        std::cout << "[DEBUG] Beginning function definition: " << funcName << " at IP: " << ip << std::endl;
    }
    ...
    if (insideClassDefinition && !currentClassBeingDefined.empty()) {
        ...
        if (debugMode) {
            std::cout << "[DEBUG] Stored function: " << funcName << " start: " << func.startAddress << std::endl;
        }
    }
...
void VM::handleEndFunction(const Instruction& /*unused*/) {
    ...
    if (!currentFunctionBeingDefined.empty()) {
        ...
        if (funcIt != userDefinedFunctions.end()) {
            ...
            if (debugMode) {
                std::cout << "[DEBUG] Completed function definition: " << currentFunctionBeingDefined 
                         << " end: " << ip << std::endl;
            }
        }
