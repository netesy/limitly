...
void FyraIRGenerator::emit_builtins() {
    ...
    auto gv_minus = new ir::GlobalVariable(ir::PointerType::get(ir::IntegerType::get(8)), minus_name);
    gv_minus->setInitializer(ir::ConstantString::get("-"));
...
ir::Value* FyraIRGenerator::emit_literal_expr(
    ...
    if (std::holds_alternative<std::string>(expr->value)) {
        ...
        if (expr->literalType == Frontend::TokenType::INT_LITERAL) {
             ...
             auto gv = new ir::GlobalVariable(ir::PointerType::get(ir::IntegerType::get(8)), name);
             gv->setInitializer(static_cast<ir::Constant*>(str_const));
             current_module_->addGlobalVariable(gv);
             return builder_->createCall(box_string, {gv});
        }
