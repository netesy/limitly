...
std::shared_ptr<ir::Module> FyraIRGenerator::generate_from_ast(
    ...
    // Process all frame declarations first to ensure types are available
    ...
    // Process all function and method declarations (forward declarations)
    for (const auto& stmt : program->statements) {
    ...
    // Create __limit_top_level__ function for top-level code
    ...
    // Implicit return 0 for top level
    ...
    // Process declarations (implementation)
    for (const auto& stmt : program->statements) {
        if (auto func_decl = std::dynamic_pointer_cast<Frontend::AST::FunctionDeclaration>(stmt)) {
    ...
    // Name allocation pass: Ensure all values have unique names
    for (auto& func : current_module_->getFunctions()) {
        int v_counter_loc = 0;
        for (auto& bb : func->getBasicBlocks()) {
            for (auto& instr : bb->getInstructions()) {
                if (instr->getType()->getTypeID() != ir::Type::VoidTyID && instr->getName().empty()) {
                    instr->setName("v" + std::to_string(v_counter_loc++));
                }
            }
        }
    }
...
ir::Function* FyraIRGenerator::generate_function(
    ...
    for (size_t i = 0; i < func_decl->params.size(); ++i, ++it) {
        std::string param_name = func_decl->params[i].first;
        ir::Type* p_type = it->get()->getType();
        // Allocate storage slot for parameter
...
ir::Value* FyraIRGenerator::emit_expression(
    ...
    if (auto binary = std::dynamic_pointer_cast<Frontend::AST::BinaryExpr>(expr)) {
        ...
        // Handle Frame instantiation
        ir::Type* type = current_module_->getType(frame_init->frameName);
        if (!type) {
            // Check if it's a built-in or externally defined
        ...
        if (type_size == 0) type_size = 16; // Fallback for 2 fields
...
ir::Value* FyraIRGenerator::emit_literal_expr(
    ...
    if (std::holds_alternative<std::string>(expr->value)) {
        ...
        if (expr->literalType == Frontend::TokenType::INT_LITERAL) {
             ...
             // String literal - return as LmBox
             ...
             return builder_->createCall(box_string, {str_const});
        }
    } else if (std::holds_alternative<bool>(expr->value)) {
        return ir::ConstantInt::get(ir::IntegerType::get(1), std::get<bool>(expr->value));
    }
    return nullptr;
}
...
ir::Value* FyraIRGenerator::emit_variable_expr(
    ...
    if (current_locals_.count(name)) {
        ir::Value* slot = current_locals_[name];
        // Always load from the storage slot to get the current value
        return builder_->createLoad(slot);
    }
    return nullptr;
}
...
ir::Value* FyraIRGenerator::emit_call_expr(
    ...
    if (auto var = std::dynamic_pointer_cast<Frontend::AST::VariableExpr>(expr->callee)) {
        func_name = var->name;
        // Check if it's actually a frame instantiation misparsed as a CallExpr
        ...
        if (auto struct_ty = dynamic_cast<ir::StructType*>(obj_ty)) {
            ...
            args.push_back(obj); // Implicit 'self'
        } else if (auto ptr_ty = dynamic_cast<ir::PointerType*>(obj_ty)) {
             if (auto inner_struct = dynamic_cast<ir::StructType*>(ptr_ty->getElementType())) {
                 func_name = inner_struct->getName() + "." + member->name;
                 args.push_back(obj);
             } else {
...
ir::Value* FyraIRGenerator::emit_assign_expr(
    ...
    if (expr->object) {
        ...
        if (auto var = std::dynamic_pointer_cast<Frontend::AST::VariableExpr>(expr->object)) {
            ...
            if (name == "this") name = "self";
            if (current_locals_.count(name)) {
                obj = builder_->createLoad(current_locals_[name]);
            }
        }
        if (!obj) obj = emit_expression(expr->object);

        if (expr->member.has_value()) {
            // Member assignment: obj.member = val
            ir::Type* obj_ty = obj->getType();
            if (auto ptr_ty = dynamic_cast<ir::PointerType*>(obj_ty)) {
                obj_ty = ptr_ty->getElementType();
            }
            if (auto struct_ty = dynamic_cast<ir::StructType*>(obj_ty)) {
                ...
                if (struct_field_offsets_.count(struct_name) && struct_field_offsets_[struct_name].count(expr->member.value())) {
                    ...
                    ir::Value* offset_val = ir::ConstantInt::get(ir::IntegerType::get(64), (long long)offset);

                    ir::Value* field_ptr = builder_->createAdd(obj, offset_val);
            ...
            if (!list_set) {
                list_set = builder_->createFunction("lm_list_set", ir::VoidType::get(), {ir::IntegerType::get(64), ir::IntegerType::get(64), ir::IntegerType::get(64)});
            }
        ...
        // Direct assignment to local variable slot
        std::string name = expr->name;
        if (name == "this") name = "self";
        if (current_locals_.count(name)) {
            ir::Value* slot = current_locals_[name];
...
ir::Value* FyraIRGenerator::emit_index_expr(
    ...
    // Basic index access for lists
    ir::Value* obj = emit_expression(expr->object);
    ir::Value* idx = emit_expression(expr->index);
    ir::Function* list_get = current_module_->getFunction("lm_list_get");
    if (!list_get) {
        list_get = builder_->createFunction("lm_list_get", ir::IntegerType::get(64), {ir::IntegerType::get(64), ir::IntegerType::get(64)});
    }
...
ir::Value* FyraIRGenerator::emit_list_expr(
    ...
    if (!list_new) {
        list_new = builder_->createFunction("lm_list_new", ir::IntegerType::get(64), {});
    }
    ir::Value* list = builder_->createCall(list_new, {});
    
    ir::Function* list_append = current_module_->getFunction("lm_list_append");
    if (!list_append) {
        list_append = builder_->createFunction("lm_list_append", ir::VoidType::get(), {ir::IntegerType::get(64), ir::IntegerType::get(64)});
    }
...
ir::Value* FyraIRGenerator::emit_dict_expr(
    ...
    if (!dict_new) {
        dict_new = builder_->createFunction("jit_dict_new", ir::IntegerType::get(64), {});
    }
    ir::Value* dict = builder_->createCall(dict_new, {});
    
    ir::Function* dict_set = current_module_->getFunction("lm_dict_set");
    if (!dict_set) {
        dict_set = builder_->createFunction("lm_dict_set", ir::VoidType::get(), {ir::IntegerType::get(64), ir::IntegerType::get(64), ir::IntegerType::get(64)});
    }
...
void FyraIRGenerator::emit_class_declaration(
    ...
    for (const auto& method : decl->methods) {
        ...
        for (size_t i = 0; i < method->parameters.size(); ++i, ++it) {
        ...
        if (method->body) {
            emit_block_statement(method->body);
        }
...
void FyraIRGenerator::emit_if_statement(
    ...
    if (builder_->getInsertPoint()->getInstructions().empty() || builder_->getInsertPoint()->getInstructions().back()->getOpcode() != ir::Instruction::Ret) {
         builder_->createJmp(merge_bb);
    }
...
void FyraIRGenerator::emit_for_statement(
    ...
    if (stmt->condition) {
        ...
        builder_->createJmp(body_bb);
    }
    
    builder_->setInsertPoint(body_bb);
    emit_statement(stmt->body);
    if (builder_->getInsertPoint()->getInstructions().empty() || builder_->getInsertPoint()->getInstructions().back()->getOpcode() != ir::Instruction::Ret) {
        builder_->createJmp(inc_bb);
    }
...
void FyraIRGenerator::emit_iter_statement(
    ...
    if (!list_len) {
        list_len = builder_->createFunction("lm_list_len", ir::IntegerType::get(64), {ir::IntegerType::get(64)});
    }
...
void FyraIRGenerator::emit_print_statement(
    ...
    if (!print_int) {
        print_int = builder_->createFunction("lm_print_int", ir::VoidType::get(), {ir::IntegerType::get(64)});
    }
...
void FyraIRGenerator::emit_return_statement(
    ...
    if (stmt->value) {
        builder_->createRet(emit_expression(stmt->value));
    } else {
        builder_->createRet(nullptr);
    }
}
